if (isNil "AWG_key_combos") then {AWG_key_combos = true;};	//use keys combinations to show and toggle options
if (isNil "AWG_show_zeds") then {AWG_show_zeds = true;};  		//Show zombies on start
if (isNil "AWG_show_animals") then {AWG_show_animals = false;};  		//Show animals on start
if (isNil "AWG_show_dead") then {AWG_show_dead = false;};  		//Show dead bodies on start
if (isNil "AWG_show_tents") then {AWG_show_tents = true;};		//Show tents on start
if (isNil "AWG_show_hud") then {AWG_show_hud = true;};		//Show players on HUD (ESP) on start
if (isNil "AWG_change_gps") then {AWG_change_gps = true;};  	//Move gps and remove it's fckn frame
if (isNil "AWG_simple_car_icons") then {AWG_simple_car_icons = false;};		//Show "simple" car icon instead default icons
if (isNil "AWG_AWG_builtin_tp") then {AWG_builtin_tp = true;};		//Use built-in teleport
if (isNil "AWG_alt_tp") then {AWG_alt_tp = false;};		//Use alternative teleport (could be used to tp on buildings)(not works for vehicles)
...
AWG_selfmarker = "You";  					//Text to identify You on map/gps. Leave blank ("") if You want to see only black icon
...
AWG_adminesp = 
{
	...
	[
		...
		[format["Built-in HUD(ESP): %1",AWG_show_hud], [2], "", -5, [["expression", "AWG_show_hud = !AWG_show_hud;"]], "1", "1"],
		[format["Built-in Teleport: %1",AWG_builtin_tp], [3], "", -5, [["expression", "AWG_builtin_tp = !AWG_builtin_tp;"]], "1", "1"],
		[format["Simple car icons: %1",AWG_simple_car_icons], [4], "", -5, [["expression", "AWG_simple_car_icons = !AWG_simple_car_icons;"]], "1", "1"],
		[format["Show tents: %1",AWG_show_tents], [5], "", -5, [["expression", "AWG_show_tents = !AWG_show_tents;"]], "1", "1"],
		[format["Show nearby zombies: %1",AWG_show_zeds], [6], "", -5, [["expression", "AWG_show_zeds = !AWG_show_zeds;"]], "1", "1"],
		[format["Show nearby animals: %1",AWG_show_animals], [7], "", -5, [["expression", "AWG_show_animals = !AWG_show_animals;"]], "1", "1"],
		[format["Show dead bodies: %1",AWG_show_dead], [8], "", -5, [["expression", "AWG_show_dead = !AWG_show_dead;"]], "1", "1"],
		[format["Use alternative teleport: %1",AWG_alt_tp], [9], "", -5, [["expression", "AWG_alt_tp = !AWG_alt_tp;"]], "1", "1"]
		];  
...
if (isnil "mapm") then 
{
	...
	if AWG_show_hud then {setGroupIconsVisible [false,true];};
...
drawic = 
{
	if (!isnil "mapm") then 
	{
		...
		if AWG_show_zeds then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\iconman_ca.paa", zmc, getPosASL _x, _iscale*30, _iscale*30, getDir _x, "", 1];};} foreach zombies;};
		if AWG_show_animals then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\icon_animals_ca.paa", amc, getPosASL _x, _iscale*15, _iscale*15, 0, "", 1];};} foreach animalsl;};
		if AWG_show_tents then {{if (!isnull _x) then {_ctrl drawIcon ["\ca\ui\data\markers\waypoint.paa", tmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach tents;};
		if AWG_show_dead then {{if ((!isnull _x) and !(_x isKindOf "zZombie_base") and (_x isKindOf "Man")) then {_ctrl drawIcon ["\ca\ui\data\map_chapel_ca.paa", amc, getPosASL _x, _iscale*30, _iscale*30, 0, "", 1];};} foreach AllDead;};
		...
		{if  (!isnull _x) then {_ctrl drawIcon ["\ca\air2\data\ui\icon_uh1y_ca.paa", wmc, getPosASL _x, 30, 30, 0, "", 1];};} foreach helicrashes;
		{
			if (!isnull _x) then 
			{
				...
				if ((AWG_simple_car_icons) and (_x iskindof "landvehicle")) then 
...
{	
	if (!isnil "mapm") then
	{				
		if ((_this select 1) and (AWG_builtin_tp)) then
		{
			_pos = _this select 0;
			if AWG_alt_tp then {_dummyveh = "PBX" createVehicleLocal [_pos select 0, _pos select 1];player moveInDriver _dummyveh;deleteVehicle _dummyveh;}
...
{	
 	...
 	if (!isnil "mapm") then 
	{
		...
		if ((visiblemap) and (AWG_builtin_tp))then 
		{
			onMapSingleClick "[_pos, _alt] call portal;";

		};

		if (AWG_show_zeds) then {zombies = getPosATL player nearEntities ["zZombie_Base", _howfarzed];} else {zombies = [];};
		if (AWG_show_animals) then {animalsl = getPosATL player nearEntities ["CAAnimalBase", _howfarzed];} else {animalsl = [];};
             
	        {
		      	if (!(_x in aList) and AWG_show_animals) then 			 
			{
				private ["_pos", "_mkr"]; 
				_pos = getPos _x;
				...
				{ 
					...
					_u = _this select 0; 
					_m = _this select 1; 
					while {mapm and AWG_show_animals and (getDammage _u < 1) } do 
					{ 
		...
		if AWG_show_dead then
		{
			      	...
			      	if (!(_x in dList) and !(_x isKindOf "zZombie_base") and (_x isKindOf "Man")) then 			 
				{
					...
					{ 
						...
						_u = _this select 0; 
						_m = _this select 1; 
						while {mapm and AWG_show_dead and (!isnull _u)} do 
						{ 
							sleep 0.5; 
						}; 
						deleteMarkerLocal _m;	
		...
		{ 
			if (!(_x in sList) and (player != _x) and (AWG_show_hud) and (getPlayerUID _x != "")) then
			{
				...
				{ 
					private ["_u"]; 
					_u = _this select 0; 
					while {(alive _u) and mapm and AWG_show_hud} do   
					{ 
                      	...
                      	if (!(_x in pList) and (vehicle _x == _x) and (getPlayerUID _x != "")) then 			 
			{
				...
				{	
					_mkr setMarkerColorLocal "ColorBlack";
					_mkr setMarkerTextLocal AWG_selfmarker;	
				};
