...
// THIS NEEDS TO BE REMOVED /\ /\ /\ For BASE BUILDING REMOVAL TO WORK
	...
	*/
	...
	// Salvage Vehicle Parts Mod BEGIN
			...
			
			{
				...
				if( _damage < 0.15 ) then {
					...
					if (!_skip ) then {
						...
						_handle = silver_myCursorTarget addAction [_string, "extras\ss_remove.sqf",[_vehicle,_part,_x], 0, false, true, "",""];
